...
**checkpoint-ai** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `checkpoint-ai` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
checkpoint-ai ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
checkpoint-ai ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
checkpoint-ai ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `checkpoint-ai` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="checkpoint-ai") for rec in records]   # records = your JSON output
